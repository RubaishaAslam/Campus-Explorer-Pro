...
public class User_Interface extends javax.swing.JFrame {
        ...
        UserCreated_POI_PanelLayout.setHorizontalGroup(
                UserCreated_POI_PanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                        .addGroup(UserCreated_POI_PanelLayout.createSequentialGroup()
                                .addContainerGap()
                                .addComponent(UC_POI_Label, javax.swing.GroupLayout.PREFERRED_SIZE, 200,
                                        javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addContainerGap(53, 53)
                                .addComponent(Create_POI_button, javax.swing.GroupLayout.PREFERRED_SIZE, 130,
                                        javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addGap(18, 18, 18)
                                .addContainerGap(javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)));
    ...
    /** 
     * This method clears the Img panel and updates it with a new floor map and all the points of interest (POIs) for that map.
     * @param evt An ActionEvent object representing the button click event
     */
    ...
    /** 
     * This method is called when the Building_ComboBox selection changes.
     * It updates the Floor_ComboBox with the floors available for the selected building.
     * @param evt An ActionEvent object representing the selection change event
     */
    ...
    /** 
     * Handles the mouse click event on the Img component. 
     * If a new POI is being created,creates a new POI at the location of the mouse click and adds it to the current floor map.
     * @param evt A MouseEvent object representing the mouse click event
     */
    ...
    /** 
     * Handles the mouse click event on the jMenu3 component. 
     * If the "Help" PDF file exists, opens it with the default desktop application. 
     * Otherwise, outputs an error message to the console.
    ...
    /** 
     * Handles the mouse-click event on the fourth menu item (jMenu4). 
     * If the Weather API response code is 200, it opens a new window displaying the weather data. 
     * If the response code is not 200, it displays a window indicating that weather data is unavailable.
     * @param evt The mouse-click event on the jMenu4 item.
     */
    ...
    /** 
     * Handles the action event on the first menu item (jMenuItem1). 
     * It serializes the model and exits the program when clicked.
     * @param evt The action event on the jMenuItem1 item.
     */
    ...
    /** 
     * This method is used to update the selected layers with the given layer name and whether it is selected or not.
     * If selected is true, the layer is added to the selectedLayers list.
     * If selected is false, the layer is removed from the selectedLayers list.
     * @param layerName The name of the layer to update.
     * @param selected True if the layer should be selected, false if it should be deselected.
     */
    ...
    /** 
     * This method is used to display the selected layers. 
     * It retrieves the building and floor map from the model, 
     * and then displays the relevant points of interest for each selected layer.
     */
    ...
    /** 
     * Overrides the method actionPerformed to perform an action event on the Layer_Button.
     * It updates the selected layers and displays the selected layers on the user interface.
     * It also switches the map when a POI is clicked and displays the matching POIs.
     * @param evt the action event to be performed.
     */
    public void Layer_ButtonActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        if (evt.getSource() == Layer_Button) {
            ...
            // turn off display for all POIs
            ArrayList<Poi> allPois = floormap.getPois();
    ...
    /** 
     * Performs an action event on the Find_Button.
     * It searches for POIs that match with the user's search input and displays the matching POIs on the user interface.
     * It also switches the map when a POI is clicked.
     * @param evt the action event to be performed.
     */
    public void Find_ButtonActionPerformed(java.awt.event.ActionEvent evt) {
        Building bld = model.getBuilding(Building_ComboBox.getSelectedItem().toString());
        FloorMap floormap = bld.getFloorMap(Floor_ComboBox.getSelectedItem().toString());
        // BufferedImage pinPicture;

        String userSearched = Search_TextField.getText(); 

        ArrayList<String> matchingPOIs = new ArrayList<String>();
        ArrayList<Poi> matchedPOIs = new ArrayList<Poi>();

        for (Poi poi : floormap.getPois()) {
            if (poi.getLocDesc().contains(userSearched)) {
                matchingPOIs.add(poi.getLabel());
                matchedPOIs.add(poi);
            }
            if (poi.getLabel().contains(userSearched)) {
                matchingPOIs.add(poi.getLabel());
                matchedPOIs.add(poi);
            }
            if (poi.getType().contains(userSearched)) {
                matchingPOIs.add(poi.getLabel());
                matchedPOIs.add(poi);
            }
            if (poi.getClassification().contains(userSearched)) {
                matchingPOIs.add(poi.getLabel());
                matchedPOIs.add(poi);
            }
        }

        // Loop through the POIs in each category and search for matching POIs
        if (matchingPOIs.isEmpty()) {
            // pop up the no matching POIs found image and class

            JFrame Invalid_Search = new Invalid_Search(model);
            Invalid_Search.setVisible(true);
            Invalid_Search.setSize(250, 250);
            Invalid_Search.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
            ...
            JFrame Search_Popup = new Search_Popup(model, matchingPOIs);
            Search_Popup.setVisible(true);
            Search_Popup.setSize(250, 250);
            Search_Popup.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
